...
def evaluate_conversation(moderator_instruction:str, moderated_user_instruction:str, conversation: List[str], engine:str="gpt-3.5-turbo"): 
    ...
    instructions = [
        {"metric": "bot_alignment", "instruction": "Given the following instructions and conversation, how well-aligned is the moderator with the instruction it was given?"},
        {"metric": "user_alignment", "instruction": "Given the following instructions and conversation, how well-aligned is the moderated user with the instruction it was given?"},
        {"metric": "coherency", "instruction": "How coherent is the moderator's response?"},
        {"metric": "understanding", "instruction": "How well does the moderator understand the moderated user?"},
        {"metric": "engaging", "instruction": "How engaging is the moderator?"},
        {"metric": "repetitiveness", "instruction": "How repetitive is the moderator?"},
        {"metric": "persuasiveness", "instruction": "How persuasive is the moderator?"},
    ]
...
def generate_conversation(moderator_bot:GPTBot, moderated_user_bot:GPTBot, init_conv: List[Dict[str,str]], n_turns: int):
    ...
    moderated_user_bot.prompt_generator.instruction.instruction_raw = moderated_user_bot.prompt_generator.instruction.instruction_raw.replace("|speaker_id|", last_speaker)
    ...
    for idx in range(n_turns): 
        ...
        if re.search(rf"^{moderator_bot.prompt_generator.title}:", mod_response['text']) is None:
            logger.warning(f"speaker name '{moderator_bot.prompt_generator.title}' not found in response: '{mod_response['text']}', adding it")            
            mod_response['text'] = f"{moderator_bot.prompt_generator.title}: {mod_response['text']}"
        ...
        if re.search(rf"^{moderated_user_bot.prompt_generator.title}:", moderated_user_response['text']) is None:
            logger.warning(f"speaker name '{moderated_user_bot.prompt_generator.title}' not found in response: '{moderated_user_response['text']}', adding it")
            moderated_user_response['text'] = f"{moderated_user_bot.prompt_generator.title}: {moderated_user_response['text']}"
    ...
    return {
        ...
        "moderated_user_instruction": str(moderated_user_bot.prompt_generator.instruction),
...
def parse_args():     
    ...
    parser.add_argument("-s", "--seed_topic_path", type=str, default="chat_topics_eng.json")
    parser.add_argument("-p", "--persona_config_path", type=str, default="persona_configs.json")
...
def main(): 
    ...
    for bot_persona in bot_ids:
        for user_persona in user_personas: 
            ...
            fp = f"self-talk_engine={args['engine']}|{bot_persona=}|{user_persona=}.json"
            ...
            for idx, topic in tqdm(enumerate(data)): 
                ...
                generated_conversation = generate_conversation(moderator_bot, moderated_user_bot, init_conv, n_turns = args["turns"])
                
                conversation = generated_conversation["init_conv"] + generated_conversation["continued_conv"]
                
                # add automatic evaluation scores 
                scores = evaluate_conversation(generated_conversation["moderator_instruction"], generated_conversation["moderated_user_instruction"], conversation, engine=args["engine"])
