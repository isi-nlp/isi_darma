...
function RatingSelector({ active, ratings, sending, ratingQuestion, ratingIndex, setRatings }) {
  // const ratingOptions = [<option key="empty_option" />, <option>some random option </option>]
  const ratingOptions = [<option key="empty_option" />].concat(
    ["Very", "Somewhat", "So-so", "Mostly not", "Not at all"].map((option_label, index) => {
      return (
  ...
  if (listRatingSelectors.length > 1) {
    ...
    return (
      <div className="response-type-module">
        <div>
          Thank you for completing the conversation. Please answer the following survey and submit your work. 
        </div>
        <br />
        <Form
          horizontal
...
function ResponseComponent({ taskConfig, appSettings, onMessageSend, active }) {
  ...
  if (n_bot_turns >= taskConfig.min_num_turns + 1) { // + 1 is needed as the BOT will first respond based on the given conversation context 
    return (
      <FinalSurvey   
